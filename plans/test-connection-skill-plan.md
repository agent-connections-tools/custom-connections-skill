...
| Q3 | Canonical "end session" API call? | `DELETE /einstein/ai-agent/v1/sessions/<sessionId>` with the **`x-session-end-reason: UserRequest`** header. The header is required — without it, the API returns 400 (`ConstraintViolationException: arg2 must not be null`). Validated empirically 2026-05-13. With the header, the API returns 200 with `{type: "SessionEnded", reason: "ClientRequest"}`. | Decision #8 + cleanup section + skill prompt Step 12. Skill always issues DELETE with the required header on exit. |
...
- **v3.3 (2026-05-13 — second post-test fix):** Live Scenario 1 test caught the session-cleanup DELETE returning 400 (`ConstraintViolationException: arg2 must not be null`). Empirically validated against test-org: the DELETE requires the `x-session-end-reason` header (value: `UserRequest`). Without it, every cleanup call fails. With it, the API returns 200 with `{type: "SessionEnded", reason: "ClientRequest"}`. Updated skill prompt Step 12, plan Q3 resolution, testing guide cleanup verification. Sessions auto-expire even without DELETE, so this was a "warning" in test output — but the right fix is using the API correctly, not relying on auto-expiration.
